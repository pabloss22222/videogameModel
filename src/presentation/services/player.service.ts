...
export class PlayerService {
  ...
  async findConstructionsOnePlayer(playerId: number){
    const player = await Player.findOne({
      where: {
        id:playerId
      },
  ...
  async findQuestsOfOnePlayer(playerId: number){
    const player = await Player.findOne({
      where: {
        id:playerId
      },
    ...
    if(!player) throw CustomError.badRequest('This player not existing')
    return player;
