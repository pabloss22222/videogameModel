...
export class PlayerController {
  ...
  findPlayerInventory = async (req: Request, res: Response)=>{
    const { id } = req.params;
    this.inventoryService.findOneInventoryByPlayerId2(+id)
      .then((resp) => res.status(200).json(resp))
      .catch(error => this.handleError(error, res))

  }

  findConstructionsOnePlayer = async (req: Request, res: Response)=>{

    const { id } = req.params;
    this.playerService.findConstructionsOnePlayer(+id)
    .then((constructions) => res.status(200).json(constructions))
    .catch(error => this.handleError(error, res))
  }
  findQuestsOfOnePlayer = async (req: Request, res: Response)=>{

    const { id } = req.params;
    this.playerService.findQuestsOfOnePlayer(+id)
    .then((resp) => res.status(200).json(resp))
    .catch(error => this.handleError(error, res))
  }
